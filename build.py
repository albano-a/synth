...
Usage:
    python build.py           standard build (clean + PyInstaller)
...
_LOGO_PNG = ROOT / "src" / "gui" / "resources" / "icons" / "logo.png"
_LOGO_ICO = ROOT / "src" / "gui" / "resources" / "icons" / "logo.ico"
...
def make_icon() -> None:
    """Convert logo.png → logo.ico so PyInstaller can embed it in the .exe."""
    if _LOGO_ICO.exists():
        return
    try:
